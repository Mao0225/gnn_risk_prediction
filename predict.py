...
from utils import build_graph_fast
...
def load_and_preprocess_data(file_path):
    ...
    try:
        ...
        print("🔄 构建图结构...")
        try:
            data = build_graph_fast(polygons, features)
...
def predict(file_path, model_path, base_out_dir):
    ...
    try:
        ...
        model = GNN(in_channels=data.x.shape[1], hidden_channels=64)
        ...
        if len(preds) != len(df_valid):
            ...
            preds = preds[:len(df_valid)] if len(preds) > len(df_valid) else np.pad(preds,
                                                                                    (0, len(df_valid) - len(preds)))
        ...
        if "风险值" in df_result.columns:
            ...
            # 添加误差百分比（绝对误差占真实风险值的比例）
            # 避免除以零，当真实值为0时设为NaN
            df_result["误差百分比"] = np.where(
                df_result["风险值"] != 0,
                (df_result["绝对差值"] / df_result["风险值"]) * 100,
                np.nan
            )
        ...
        if "风险值" in df_result.columns:
            ...
            # 计算平均绝对百分比误差 (MAPE)
            # 过滤掉真实值为0的情况，避免除以0
            valid_indices = true_values != 0
            if np.any(valid_indices):
                mape = np.mean(
                    np.abs((true_values[valid_indices] - preds[valid_indices]) / true_values[valid_indices])) * 100
            else:
                mape = np.nan
                print("⚠️ 所有真实值均为0，无法计算MAPE")

            # 计算平均误差百分比（基于已计算的误差百分比列）
            if "误差百分比" in df_result.columns:
                # 过滤掉NaN值（即真实值为0的情况）
                valid_error_percentages = df_result["误差百分比"].dropna()
                if len(valid_error_percentages) > 0:
                    mean_error_percentage = np.mean(valid_error_percentages)
                else:
                    mean_error_percentage = np.nan
                    print("⚠️ 所有真实值均为0，无法计算平均误差百分比")
            else:
                mean_error_percentage = np.nan
            ...
            print(f"   平均误差百分比: {mean_error_percentage:.2f}%")
...
if __name__ == "__main__":
    ...
    try:
        ...
        parser.add_argument("--data", type=str, help="输入Excel数据文件路径", default="data/渔网完整.xlsx")
